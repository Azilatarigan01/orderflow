                            ...
                            <tbody id="itemsBody" class="divide-y divide-slate-100 font-normal">
                                @foreach($purchaseRequest->items as $idx => $item)
                                    <tr class="item-row hover:bg-slate-50/50">
                                        ...
                                        <td class="px-3 py-2">
                                            <input type="text" inputmode="numeric" name="items[{{ $idx }}][estimated_unit_price]" value="{{ number_format($item->estimated_unit_price, 0, ',', '.') }}" placeholder="Cth: 15.000.000" required
